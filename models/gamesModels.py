...
def search_games(title: str):
    """
    # Kenneth
    Queries the database for a given game name.
    Returns the top 5 games that are closely related to the given search.
    Make sure your front end has a debouncer so we are not fetching
...
def get_trending_games():
    ...
    try:
        ...
        all_games = db.query(Games).filter(
            Games.metacriticScore.isnot(None)).all()
